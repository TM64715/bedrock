...
function Auth({
  login, register, name, setName, email, setEmail, password, setPassword, onSubmit, error,
}) {
  return (
    <div className="flex items-center justify-center w-full h-screen" style={{ backgroundImage: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' }}>
      <div className={`flex flex-col items-center w-4/12 h-auto ${login ? '-mt-40' : ''} space-y-10 bg-white py-14 rounded-xl`}>
        ...
        <form className="w-6/12 space-y-6" onSubmit={onSubmit}>
          {register
              && <Input id="name" label="Name" placeholder="Tage" stacked type="text" value={name} onChange={(e) => setName(e.target.value)} border />}
          ...
          <button className="block px-10 mx-auto mt-5 btn btn-indigo" type="submit">
